...
const CipherAlgo cast256CipherAlgo =
{
   ...
   CAST256_BLOCK_SIZE,
   (CipherAlgoInit) cast256Init,
   NULL,
   NULL,
   (CipherAlgoEncryptBlock) cast256EncryptBlock,
   (CipherAlgoDecryptBlock) cast256DecryptBlock,
   (CipherAlgoDeinit) cast256Deinit
};


/**
 * @brief Key expansion
 * @param[in] context Pointer to the CAST-256 context to initialize
...
error_t cast256Init(Cast256Context *context, const uint8_t *key, size_t keyLen)
{
   ...
   if(keyLen < 16 || keyLen > 32 || (keyLen % 4) != 0)
      return ERROR_INVALID_KEY_LENGTH;

   //Let ABCDEFGH be the primary key K
   a = LOAD32BE(key);
   b = LOAD32BE(key + 4);
   c = LOAD32BE(key + 8);
   d = LOAD32BE(key + 12);

   //For key sizes less than 256 bits, the key is padded with zero bytes
   e = (keyLen >= 20) ? LOAD32BE(key + 16) : 0;
   f = (keyLen >= 24) ? LOAD32BE(key + 20) : 0;
   g = (keyLen >= 28) ? LOAD32BE(key + 24) : 0;
   h = (keyLen >= 32) ? LOAD32BE(key + 28) : 0;

   //Key schedule
   for(i = 0; i < 12; i++)
   {
      //Derive subkeys (refer to RFC 2612, section 2.4)
      W(a, b, c, d, e, f, g, h, tr[(2 * i) % 4], tm[2 * i]);
      W(a, b, c, d, e, f, g, h, tr[(2 * i + 1) % 4], tm[2 * i + 1]);

      //Save rotate subkeys Kr (only the 5 least significant bits are used)
      context->kr[i][0] = a & 0x1F;
      context->kr[i][1] = c & 0x1F;
      context->kr[i][2] = e & 0x1F;
      context->kr[i][3] = g & 0x1F;

      //Save masking subkeys Km
      context->km[i][0] = h;
      context->km[i][1] = f;
      context->km[i][2] = d;
      context->km[i][3] = b;
...
/**
 * @brief Encrypt a 8-byte block using CAST-256 algorithm
 * @param[in] context Pointer to the CAST-256 context
   ...
   uint8_t *output)
{
   ...
   //CAST-256 has 48 rounds in 12 quad-rounds
   Q(a, b, c, d, context->kr[0], context->km[0]);
   Q(a, b, c, d, context->kr[1], context->km[1]);
   Q(a, b, c, d, context->kr[2], context->km[2]);
   Q(a, b, c, d, context->kr[3], context->km[3]);
   Q(a, b, c, d, context->kr[4], context->km[4]);
   Q(a, b, c, d, context->kr[5], context->km[5]);

   QBAR(a, b, c, d, context->kr[6], context->km[6]);
   QBAR(a, b, c, d, context->kr[7], context->km[7]);
   QBAR(a, b, c, d, context->kr[8], context->km[8]);
   QBAR(a, b, c, d, context->kr[9], context->km[9]);
   QBAR(a, b, c, d, context->kr[10], context->km[10]);
...
/**
 * @brief Decrypt a 8-byte block using CAST-256 algorithm
 * @param[in] context Pointer to the CAST-256 context
   ...
   uint8_t *output)
{
   ...
   //Decryption is identical to encryption except that the sets of quad-round
   //keys Kr and Km derived from K are used in reverse order
   Q(a, b, c, d, context->kr[11], context->km[11]);
   Q(a, b, c, d, context->kr[10], context->km[10]);
   Q(a, b, c, d, context->kr[9], context->km[9]);
   Q(a, b, c, d, context->kr[8], context->km[8]);
   Q(a, b, c, d, context->kr[7], context->km[7]);
   Q(a, b, c, d, context->kr[6], context->km[6]);

   QBAR(a, b, c, d, context->kr[5], context->km[5]);
   QBAR(a, b, c, d, context->kr[4], context->km[4]);
   QBAR(a, b, c, d, context->kr[3], context->km[3]);
   QBAR(a, b, c, d, context->kr[2], context->km[2]);
   QBAR(a, b, c, d, context->kr[1], context->km[1]);
...
/**
 * @brief Release CAST-256 context
 * @param[in] context Pointer to the CAST-256 context
 **/

void cast256Deinit(Cast256Context *context)
{
   //Clear CAST-256 context
   osMemset(context, 0, sizeof(Cast256Context));
}
