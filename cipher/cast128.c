...
const CipherAlgo cast128CipherAlgo =
{
   ...
   CAST128_BLOCK_SIZE,
   (CipherAlgoInit) cast128Init,
   NULL,
   NULL,
   (CipherAlgoEncryptBlock) cast128EncryptBlock,
   (CipherAlgoDecryptBlock) cast128DecryptBlock,
   (CipherAlgoDeinit) cast128Deinit
};


/**
 * @brief Key expansion
 * @param[in] context Pointer to the CAST-128 context to initialize
...
error_t cast128Init(Cast128Context *context, const uint8_t *key, size_t keyLen)
{
   ...
   //that can vary from 40 bits to 128 bits, in 8-bit increments (refer to
   //RFC 2144, section 2.5)
   if(keyLen < 5 || keyLen > 16)
      return ERROR_INVALID_KEY_LENGTH;

   //For key sizes up to and including 80 bits, the algorithm uses 12 rounds
   //instead of 16
   context->nr = (keyLen <= 10) ? 12 : 16;
   ...
   for(i = 0; i < 32; i += 16)
   {
      //CAST-128 uses a pair of subkeys per round: a 32-bit quantity Km is used
      //as a "masking" key and a 5-bit quantity Kr is used as a "rotation" key
      if(i == 0)
      {
         k = context->km;
      }
      ...
      //Derive subkeys (refer to RFC 2144, section 2.4)
      ...
      k[12] = S5(x2, 3) ^ S6(x2, 2) ^ S7(x1, 0) ^ S8(x1, 1) ^ S5(x0, 0);
      k[13] = S5(x2, 1) ^ S6(x2, 0) ^ S7(x1, 2) ^ S8(x1, 3) ^ S6(x1, 0);
      k[14] = S5(x3, 3) ^ S6(x3, 2) ^ S7(x0, 0) ^ S8(x0, 1) ^ S7(x2, 3);
      k[15] = S5(x3, 1) ^ S6(x3, 0) ^ S7(x0, 2) ^ S8(x0, 3) ^ S8(x3, 2);
   }
...
/**
 * @brief Encrypt a 8-byte block using CAST-128 algorithm
 * @param[in] context Pointer to the CAST-128 context
   ...
   uint8_t *output)
{
   ...
   F2(r, l, context->kr[1], context->km[1]);
   F3(l, r, context->kr[2], context->km[2]);
   F1(r, l, context->kr[3], context->km[3]);
   F2(l, r, context->kr[4], context->km[4]);
   F3(r, l, context->kr[5], context->km[5]);
   F1(l, r, context->kr[6], context->km[6]);
   F2(r, l, context->kr[7], context->km[7]);
   F3(l, r, context->kr[8], context->km[8]);
   F1(r, l, context->kr[9], context->km[9]);
   F2(l, r, context->kr[10], context->km[10]);
   F3(r, l, context->kr[11], context->km[11]);

   //For key sizes greater than 80 bits, the algorithm uses the full 16 rounds
   if(context->nr == 16)
   {
      F1(l, r, context->kr[12], context->km[12]);
      F2(r, l, context->kr[13], context->km[13]);
      F3(l, r, context->kr[14], context->km[14]);
      F1(r, l, context->kr[15], context->km[15]);
   }
...
/**
 * @brief Decrypt a 8-byte block using CAST-128 algorithm
 * @param[in] context Pointer to the CAST-128 context
...
/**
 * @brief Release CAST-128 context
 * @param[in] context Pointer to the CAST-128 context
 **/

void cast128Deinit(Cast128Context *context)
{
   //Clear CAST-128 context
   osMemset(context, 0, sizeof(Cast128Context));
}
