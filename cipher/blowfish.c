 ...
 * @section Description
 *
 * Blowfish is an encryption algorithm designed to encipher and decipher
...
//S-box 4
static const uint32_t sbox4[256] =
{
...
{
   "Blowfish",
   sizeof(BlowfishContext),
   CIPHER_ALGO_TYPE_BLOCK,
   BLOWFISH_BLOCK_SIZE,
   (CipherAlgoInit) blowfishInit,
   NULL,
   NULL,
...
/**
 * @brief Initialize a Blowfish context using the supplied key
 * @param[in] context Pointer to the Blowfish context to initialize
   ...
   size_t keyLen)
{
   ...
   if(!error)
   {
      ...
      error = blowfishExpandKey(context, NULL, 0, key, keyLen);
   }
...
/**
 * @brief Blowfish state initialization
...
error_t blowfishInitState(BlowfishContext *context)
{
   ...
   //Blowfish uses an initial P-array derived from the hexadecimal digits of pi
   for(i = 0; i < 18; i++)
   {
      context->p[i] = parray[i];
   }

   //Blowfish uses initial S-boxes derived from the hexadecimal digits of pi
   for(i = 0; i < 256; i++)
   {
      context->s1[i] = sbox1[i];
      context->s2[i] = sbox2[i];
      context->s3[i] = sbox3[i];
      context->s4[i] = sbox4[i];
   }
   ...
   size_t saltLen, const uint8_t *key, size_t keyLen)
{
   ...
   size_t keyIndex;
   size_t saltIndex;
   uint8_t data[BLOWFISH_BLOCK_SIZE];

   //Check the length of the salt
   if(saltLen != 0 && saltLen < BLOWFISH_BLOCK_SIZE)
      return ERROR_INVALID_PARAMETER;
   ...
   //In the first phase of setting up the P-array, the secret key K is
   //cyclically XOR'd into the P-array
   for(i = 0; i < 18; i++)
   {
      //This is done by taking successive 32-bit quantities of K
      for(value = 0, j = 0; j < 4; j++)
      {
         //Read current byte
         value = (value << 8) | key[keyIndex];

         //Wrap around to the start if the end of the key is reached
         if(++keyIndex >= keyLen)
         {
            keyIndex = 0;
         }
      ...
      //XOR the 32-bit quantities of K with the P-array elements
      context->p[i] ^= value;
   }
   ...
   for(i = 0; i < 18; i += 2)
   {
      //The data block is XOR'd with the salt
      blowfishXorBlock(data, salt, saltLen, &saltIndex);

      //Encrypt the data block
      blowfishEncryptBlock(context, data, data);
      context->p[i] = LOAD32BE(data);
   ...
   for(i = 0; i < 256; i += 2)
   {
      ...
      //Encrypt the data block
      blowfishEncryptBlock(context, data, data);
      context->s1[i] = LOAD32BE(data);
      context->s1[i + 1] = LOAD32BE(data + 4);
   }
   ...
   for(i = 0; i < 256; i += 2)
   {
      ...
      //Encrypt the data block
      blowfishEncryptBlock(context, data, data);
      context->s2[i] = LOAD32BE(data);
      context->s2[i + 1] = LOAD32BE(data + 4);
   }
   ...
   for(i = 0; i < 256; i += 2)
   {
      ...
      //Encrypt the data block
      blowfishEncryptBlock(context, data, data);
      context->s3[i] = LOAD32BE(data);
      context->s3[i + 1] = LOAD32BE(data + 4);
   }
   ...
   for(i = 0; i < 256; i += 2)
   {
      ...
      //Encrypt the data block
      blowfishEncryptBlock(context, data, data);
      context->s4[i] = LOAD32BE(data);
      context->s4[i + 1] = LOAD32BE(data + 4);
   }
...
/**
 * @brief XOR data block with random salt
 * @param[in,out] data Pointer the data block (16 bytes)
 * @param[in] salt Pointer to the random salt (16 bytes)
 * @param[in] saltLen Length of the salt, in bytes
 * @param[in,out] saltIndex Current salt index
 **/
   ...
   size_t *saltIndex)
{
   ...
   if(saltLen > 0)
   {
      //Get current salt index
      n = *saltIndex;

      //XOR the data block with the random salt
      for(i = 0; i < BLOWFISH_BLOCK_SIZE; i++)
      {
         //Perform XOR operation
         data[i] ^= salt[n];

         //Treat the salt as cyclic
         if(++n >= saltLen)
         {
            n = 0;
         }
