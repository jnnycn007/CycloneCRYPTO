/**
 * @file pic32cx_mt_crypto_cipher.c
...
#if (PIC32CX_MT_CRYPTO_CIPHER_SUPPORT == ENABLED && AES_SUPPORT == ENABLED)
   ...
   uint8_t *output, size_t length, uint32_t mode)
{
   ...
   //Set operation mode
   AES_REGS->AES_MR = AES_MR_SMOD_AUTO_START | mode;
   //Set encryption key
   aesLoadKey(context);
   ...
   size_t length, uint8_t *t, uint32_t mode)
{
   ...
   if(aLen > 0 || length > 0)
   {
      ...
      AES_REGS->AES_MR |= AES_MR_SMOD_AUTO_START | AES_MR_OPMOD_GCM |
   ...
   else
   {
      ...
      AES_REGS->AES_MR |= AES_MR_SMOD_AUTO_START | AES_MR_OPMOD_CTR |
         AES_MR_CIPHER_Msk;
