...
#include "hardware/m480/m480_crypto_cipher.h"
...
#if (M480_CRYPTO_CIPHER_SUPPORT == ENABLED)
...
void aesLoadKey(AesContext *context)
{
   ...
   if(context->nr == 10)
   {
      ...
      CRPT->AES0_KEY[0] = context->ek[0];
      CRPT->AES0_KEY[1] = context->ek[1];
      CRPT->AES0_KEY[2] = context->ek[2];
      CRPT->AES0_KEY[3] = context->ek[3];
   }
   else if(context->nr == 12)
   {
      ...
      CRPT->AES0_KEY[2] = context->ek[2];
      CRPT->AES0_KEY[3] = context->ek[3];
      CRPT->AES0_KEY[4] = context->ek[4];
      CRPT->AES0_KEY[5] = context->ek[5];
   }
   else
   {
      ...
      CRPT->AES0_KEY[4] = context->ek[4];
      CRPT->AES0_KEY[5] = context->ek[5];
      CRPT->AES0_KEY[6] = context->ek[6];
      CRPT->AES0_KEY[7] = context->ek[7];
   }
}
...
void aesProcessDataBlock(const uint8_t *input, uint8_t *output)
{
   ...
   //Save feedback information
   CRPT->AES0_IV[0] = CRPT->AES_FDBCK[0];
   CRPT->AES0_IV[1] = CRPT->AES_FDBCK[1];
   CRPT->AES0_IV[2] = CRPT->AES_FDBCK[2];
   CRPT->AES0_IV[3] = CRPT->AES_FDBCK[3];
}
   ...
   uint8_t *output, size_t length, uint32_t opmode)
{
   ...
   {
      //Set initialization vector
      CRPT->AES0_IV[0] = LOAD32BE(iv);
      CRPT->AES0_IV[1] = LOAD32BE(iv + 4);
      CRPT->AES0_IV[2] = LOAD32BE(iv + 8);
      CRPT->AES0_IV[3] = LOAD32BE(iv + 12);
   }
