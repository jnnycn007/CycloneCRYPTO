/**
 * @file sam4c_crypto_cipher.c
 * @brief SAM4C cipher hardware accelerator
 *
 * @section License
 *
...
#if (SAM4C_CRYPTO_CIPHER_SUPPORT == ENABLED && AES_SUPPORT == ENABLED)
   ...
   uint8_t *output, size_t length, uint32_t mode)
{
   ...
   osAcquireMutex(&sam4cCryptoMutex);

   //Perform software reset
   AES->AES_CR = AES_CR_SWRST;

   //Set operation mode
   AES->AES_MR = AES_MR_SMOD_AUTO_START | mode;
   //Set encryption key
   aesLoadKey(context);
   ...
   osReleaseMutex(&sam4cCryptoMutex);
}
   ...
   if((m % 8) == 0 && m <= (cipher->blockSize * 8))
   {
      ...
      if(cipher == AES_CIPHER_ALGO)
      {
         ...
         while(length > 0)
         {
            ...
            //Encrypt payload data
            aesProcessData(context, t, p, c, n, AES_MR_CIPHER |
               AES_MR_OPMOD_CTR);
   ...
   size_t length, uint8_t *t, uint32_t mode)
{
   ...
   if(aLen > 0 || length > 0)
   {
      ...
      AES->AES_MR |= AES_MR_SMOD_AUTO_START | AES_MR_OPMOD_GCM |
         AES_MR_GTAGEN | mode;
   ...
   else
   {
      ...
      AES->AES_MR |= AES_MR_SMOD_AUTO_START | AES_MR_OPMOD_CTR |
         AES_MR_CIPHER;
