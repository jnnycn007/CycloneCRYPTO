/**
 * @file stm32mp13xx_crypto_cipher.c
 * @brief STM32MP13 cipher hardware accelerator
 *
 * @section License
 *
   ...
   uint8_t *output, size_t length, uint32_t mode)
{
   ...
   CRYP1->CR = CRYP_CR_DATATYPE_8B;
   //Configure the algorithm and chaining mode
   CRYP1->CR |= mode;

   //Set encryption key
   CRYP1->K1LR = context->ks[0];
   CRYP1->K1RR = context->ks[1];

   //Valid initialization vector?
   if(iv != NULL)
   {
      //Set initialization vector
      CRYP1->IV0LR = LOAD32BE(iv);
      CRYP1->IV0RR = LOAD32BE(iv + 4);
   }

   //Flush the input and output FIFOs
   CRYP1->CR |= CRYP_CR_FFLUSH;
   //Enable the cryptographic processor
   CRYP1->CR |= CRYP_CR_CRYPEN;

   //Process data
   while(length >= DES_BLOCK_SIZE)
   {
      //Wait for the input FIFO to be ready to accept data
      while((CRYP1->SR & CRYP_SR_IFNF) == 0)
      {
      }

      //Write the input FIFO
      CRYP1->DIN = LOAD32LE(input);
      CRYP1->DIN = LOAD32LE(input + 4);

      //Wait for the output to be ready
      while((CRYP1->SR & CRYP_SR_OFNE) == 0)
      {
      }
   ...
   uint8_t *output, size_t length, uint32_t mode)
{
   ...
   //Set encryption key
   CRYP1->K1LR = context->k1.ks[0];
   CRYP1->K1RR = context->k1.ks[1];
   CRYP1->K2LR = context->k2.ks[0];
   CRYP1->K2RR = context->k2.ks[1];
   CRYP1->K3LR = context->k3.ks[0];
   CRYP1->K3RR = context->k3.ks[1];
...
void aesLoadKey(AesContext *context)
{
   ...
   temp = CRYP1->CR & ~CRYP_CR_KEYSIZE;

   //Check the length of the key
   if(context->nr == 10)
   {
      //10 rounds are required for 128-bit key
      CRYP1->CR = temp | CRYP_CR_KEYSIZE_128B;

      //Set the 128-bit encryption key
      CRYP1->K2LR = context->ek[0];
      CRYP1->K2RR = context->ek[1];
      CRYP1->K3LR = context->ek[2];
      CRYP1->K3RR = context->ek[3];
   }
   else if(context->nr == 12)
   {
      //12 rounds are required for 192-bit key
      CRYP1->CR = temp | CRYP_CR_KEYSIZE_192B;

      //Set the 192-bit encryption key
      CRYP1->K1LR = context->ek[0];
      CRYP1->K1RR = context->ek[1];
      CRYP1->K2LR = context->ek[2];
      CRYP1->K2RR = context->ek[3];
      CRYP1->K3LR = context->ek[4];
      CRYP1->K3RR = context->ek[5];
   }
   else
   {
      //14 rounds are required for 256-bit key
      CRYP1->CR = temp | CRYP_CR_KEYSIZE_256B;

      //Set the 256-bit encryption key
      CRYP1->K0LR = context->ek[0];
      CRYP1->K0RR = context->ek[1];
      CRYP1->K1LR = context->ek[2];
      CRYP1->K1RR = context->ek[3];
      CRYP1->K2LR = context->ek[4];
      CRYP1->K2RR = context->ek[5];
      CRYP1->K3LR = context->ek[6];
      CRYP1->K3RR = context->ek[7];
   }
}
   ...
   uint8_t *output, size_t length, uint32_t mode)
{
   ...
   if((mode & CRYP_CR_ALGODIR) != 0)
   {
      ...
      CRYP1->CR |= CRYP_CR_ALGOMODE_AES_KEY;
      //Set encryption key
      aesLoadKey(context);
   ...
   if(iv != NULL)
   {
      ...
      CRYP1->IV0LR = LOAD32BE(iv);
      CRYP1->IV0RR = LOAD32BE(iv + 4);
      CRYP1->IV1LR = LOAD32BE(iv + 8);
      CRYP1->IV1RR = LOAD32BE(iv + 12);
   }
   ...
   while(length >= AES_BLOCK_SIZE)
   {
      ...
      CRYP1->DIN = LOAD32LE(input);
      CRYP1->DIN = LOAD32LE(input + 4);
      CRYP1->DIN = LOAD32LE(input + 8);
      CRYP1->DIN = LOAD32LE(input + 12);
   ...
   size_t length, uint8_t *t, uint32_t mode)
{
   ...
   temp = CRYP1->CR & ~CRYP_CR_ALGOMODE;
   CRYP1->CR = temp | CRYP_CR_ALGOMODE_AES_GCM;

   //Configure GCM_CCMPH bits to '00' to start the GCM init phase
   temp = CRYP1->CR & ~CRYP_CR_GCM_CCMPH;
   CRYP1->CR = temp | CRYP_CR_GCM_CCMPH_INIT;
   ...
   //next phase
   while((CRYP1->CR & CRYP_CR_CRYPEN) != 0)
   {
   }

   //Configure GCM_CCMPH bits to '01' to start GCM header phase
   temp = CRYP1->CR & ~CRYP_CR_GCM_CCMPH;
   CRYP1->CR = temp | CRYP_CR_GCM_CCMPH_HEADER;
   ...
   for(n = aLen; n >= AES_BLOCK_SIZE; n -= AES_BLOCK_SIZE)
   {
      ...
      //Write the input FIFO
      CRYP1->DIN = LOAD32LE(a);
      CRYP1->DIN = LOAD32LE(a + 4);
      CRYP1->DIN = LOAD32LE(a + 8);
      CRYP1->DIN = LOAD32LE(a + 12);
   ...
   temp = CRYP1->CR & ~CRYP_CR_GCM_CCMPH;
   CRYP1->CR = temp | CRYP_CR_GCM_CCMPH_PAYLOAD;

   //Select the algorithm direction by using the ALGODIR bit
   temp = CRYP1->CR & ~CRYP_CR_ALGODIR;
   CRYP1->CR |= mode;
   ...
   if(n > 0)
   {
      ...
      temp = CRYP1->CR & ~CRYP_CR_NPBLB;
      CRYP1->CR = temp | ((AES_BLOCK_SIZE - n) << CRYP_CR_NPBLB_Pos);
   ...
   temp = CRYP1->CR & ~CRYP_CR_GCM_CCMPH;
   CRYP1->CR = temp | CRYP_CR_GCM_CCMPH_FINAL;
   ...
   uint8_t *t, uint32_t mode)
{
   ...
   temp = CRYP1->CR & ~CRYP_CR_ALGOMODE;
   CRYP1->CR = temp | CRYP_CR_ALGOMODE_AES_CCM;
   ...
   CRYP1->IV0LR = LOAD32BE(buffer);
   CRYP1->IV0RR = LOAD32BE(buffer + 4);
   CRYP1->IV1LR = LOAD32BE(buffer + 8);
   CRYP1->IV1RR = LOAD32BE(buffer + 12);

   //Set the CRYPEN bit to 1 in CRYP_CR to start accepting data
   CRYP1->CR |= CRYP_CR_CRYPEN;

   //Write the B0 packet into CRYP_DIN register
   CRYP1->DIN = LOAD32BE(b0);
   CRYP1->DIN = LOAD32BE(b0 + 4);
   CRYP1->DIN = LOAD32BE(b0 + 8);
   CRYP1->DIN = LOAD32BE(b0 + 12);
   ...
   if(aLen > 0)
   {
      ...
      //Write the input FIFO
      CRYP1->DIN = LOAD32LE(buffer);
      CRYP1->DIN = LOAD32LE(buffer + 4);
      CRYP1->DIN = LOAD32LE(buffer + 8);
      CRYP1->DIN = LOAD32LE(buffer + 12);
   ...
   if(length > 0)
   {
      ...
      //Specify the number of padding bytes in the last block
      temp = CRYP1->CR & ~CRYP_CR_NPBLB;
      CRYP1->CR = temp | ((AES_BLOCK_SIZE - length) << CRYP_CR_NPBLB_Pos);
   ...
   temp = CRYP1->CR & ~(CRYP_CR_GCM_CCMPH | CRYP_CR_ALGODIR);
   CRYP1->CR = temp | CRYP_CR_GCM_CCMPH_FINAL;
   ...
   CRYP1->DIN = LOAD32BE(buffer);
   CRYP1->DIN = LOAD32BE(buffer + 4);
   CRYP1->DIN = LOAD32BE(buffer + 8);
   CRYP1->DIN = LOAD32BE(buffer + 12);
