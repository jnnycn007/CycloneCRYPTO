...
#include "hardware/sams70/sams70_crypto.h"
#include "hardware/sams70/sams70_crypto_hash.h"
#include "hash/hash_algorithms.h"
#include "debug.h"

//Check crypto library configuration
#if (SAMS70_CRYPTO_HASH_SUPPORT == ENABLED)
...
#pragma location = SAMS70_ICM_RAM_SECTION
static Sams70IcmDesc icmDesc;
//ICM data buffer
#pragma data_alignment = 4
#pragma location = SAMS70_ICM_RAM_SECTION
static uint8_t icmBuffer[SAMS70_ICM_BUFFER_SIZE];
//ICM hash area
...
//ICM region descriptor
static Sams70IcmDesc icmDesc
   __attribute__((aligned(64), __section__(SAMS70_ICM_RAM_SECTION)));
//ICM data buffer
static uint8_t icmBuffer[SAMS70_ICM_BUFFER_SIZE]
   __attribute__((aligned(4), __section__(SAMS70_ICM_RAM_SECTION)));
//ICM hash area
static uint32_t icmHash[8]
   __attribute__((aligned(128), __section__(SAMS70_ICM_RAM_SECTION)));
   ...
   uint32_t *h)
{
   ...
   osAcquireMutex(&sams70CryptoMutex);

   //Digest input data
   while(length >= 64)
   {
      //Limit the number of data to process at a time
      n = MIN(length, SAMS70_ICM_BUFFER_SIZE);
   ...
   osReleaseMutex(&sams70CryptoMutex);
}
