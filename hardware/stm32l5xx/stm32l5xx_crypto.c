/**
 * @file stm32l5xx_crypto.c
 * @brief STM32L5 hardware cryptographic accelerator
 *
 * @section License
 *
...
#include "hardware/stm32l5xx/stm32l5xx_crypto_trng.h"
#include "hardware/stm32l5xx/stm32l5xx_crypto_hash.h"
...
error_t stm32l5xxCryptoInit(void)
{
   error_t error;
   ...
   if(!osCreateMutex(&stm32l5xxCryptoMutex))
   {
      //Failed to create mutex
      error = ERROR_OUT_OF_RESOURCES;
   }

#if (STM32L5XX_CRYPTO_TRNG_SUPPORT == ENABLED)
   //Check status code
   if(!error)
   {
...
#if (STM32L5XX_CRYPTO_HASH_SUPPORT == ENABLED)
   //Check status code
   if(!error)
   {
