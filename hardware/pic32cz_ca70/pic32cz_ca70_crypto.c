/**
 * @file pic32cz_ca70_crypto.c
 * @brief PIC32CZ CA70 hardware cryptographic accelerator
 *
 * @section License
 *
...
#include "hardware/pic32cz_ca70/pic32cz_ca70_crypto_trng.h"
...
error_t pic32czca70CryptoInit(void)
{
   ...
   if(!osCreateMutex(&pic32czca70CryptoMutex))
   {
      //Failed to create mutex
      error = ERROR_OUT_OF_RESOURCES;
   }

#if (PIC32CZ_CA70_CRYPTO_TRNG_SUPPORT == ENABLED)
   //Check status code
   if(!error)
   {
...
#if (PIC32CZ_CA70_CRYPTO_CIPHER_SUPPORT == ENABLED)
   //Check status code
   if(!error)
   {
