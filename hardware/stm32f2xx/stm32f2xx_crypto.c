...
#include "hardware/stm32f2xx/stm32f2xx_crypto_trng.h"
...
error_t stm32f2xxCryptoInit(void)
{
   ...
   if(!osCreateMutex(&stm32f2xxCryptoMutex))
   {
      //Failed to create mutex
      error = ERROR_OUT_OF_RESOURCES;
   }

#if (STM32F2XX_CRYPTO_TRNG_SUPPORT == ENABLED)
   //Check status code
   if(!error)
   {
