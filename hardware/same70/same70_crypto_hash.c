...
#pragma location = SAME70_ICM_RAM_SECTION
static Same70IcmDesc icmDesc;
//ICM data buffer
#pragma data_alignment = 4
#pragma location = SAME70_ICM_RAM_SECTION
static uint8_t icmBuffer[SAME70_ICM_BUFFER_SIZE];
//ICM hash area
...
//ICM region descriptor
static Same70IcmDesc icmDesc
   __attribute__((aligned(64), __section__(SAME70_ICM_RAM_SECTION)));
//ICM data buffer
static uint8_t icmBuffer[SAME70_ICM_BUFFER_SIZE]
   __attribute__((aligned(4), __section__(SAME70_ICM_RAM_SECTION)));
//ICM hash area
static uint32_t icmHash[8]
   __attribute__((aligned(128), __section__(SAME70_ICM_RAM_SECTION)));
   ...
   uint32_t *h)
{
   ...
   while(length >= 64)
   {
      ...
      n = MIN(length, SAME70_ICM_BUFFER_SIZE);
