...
#include "hardware/pic32cm_ls/pic32cm_ls_crypto_trng.h"
#include "debug.h"
...
error_t pic32cmlsCryptoInit(void)
{
   ...
   if(!osCreateMutex(&pic32cmlsCryptoMutex))
   {
      //Failed to create mutex
      error = ERROR_OUT_OF_RESOURCES;
   }

#if (PIC32CM_LS_CRYPTO_TRNG_SUPPORT == ENABLED)
   //Check status code
   if(!error)
   {
