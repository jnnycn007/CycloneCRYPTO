/**
 * @file mimxrt1060_crypto_cipher.c
...
static uint8_t dcpBufferIn[MIMXRT1060_DCP_BUFFER_SIZE];

//DCP output buffer
#pragma data_alignment = 16
#pragma location = MIMXRT1060_DCP_RAM_SECTION
static uint8_t dcpBufferOut[MIMXRT1060_DCP_BUFFER_SIZE];
      ...
      else if((length % AES_BLOCK_SIZE) == 0)
      {
         ...
         if(aesContext->nr == 10)
         {
            ...
            for(i = 0; i < length && status == kStatus_Success; i += n)
            {
               ...
               n = MIN(length - i, MIMXRT1060_DCP_BUFFER_SIZE);
