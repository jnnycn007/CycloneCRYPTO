...
#include "hardware/gd32g5xx/gd32g5xx_crypto.h"
#include "hardware/gd32g5xx/gd32g5xx_crypto_cipher.h"
...
#if (GD32G5XX_CRYPTO_CIPHER_SUPPORT == ENABLED)
   ...
   uint8_t *output, size_t length, uint32_t mode)
{
   ...
   osAcquireMutex(&gd32g5xxCryptoMutex);
   ...
   osReleaseMutex(&gd32g5xxCryptoMutex);
}
   ...
   uint8_t *t, uint32_t mode)
{
   ...
   CAU_DI = LOAD32BE(b0);
   CAU_DI = LOAD32BE(b0 + 4);
   CAU_DI = LOAD32BE(b0 + 8);
   CAU_DI = LOAD32BE(b0 + 12);
   ...
   CAU_DI = LOAD32BE(buffer);
   CAU_DI = LOAD32BE(buffer + 4);
   CAU_DI = LOAD32BE(buffer + 8);
   CAU_DI = LOAD32BE(buffer + 12);
