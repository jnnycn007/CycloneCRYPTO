/**
 * @file pic32cx_sg_crypto_cipher.c
...
#if (PIC32CX_SG_CRYPTO_CIPHER_SUPPORT == ENABLED && AES_SUPPORT == ENABLED)
   ...
   uint8_t *output, size_t length, uint32_t mode)
{
   ...
   osAcquireMutex(&pic32cxsgCryptoMutex);
   ...
   osReleaseMutex(&pic32cxsgCryptoMutex);
}
   ...
   if((m % 8) == 0 && m <= (cipher->blockSize * 8))
   {
      ...
      if(cipher == AES_CIPHER_ALGO)
      {
         ...
         while(length > 0)
         {
            ...
            //Encrypt payload data
            aesProcessData(context, t, p, c, n, AES_CTRLA_CIPHER_ENC |
               AES_CTRLA_AESMODE_COUNTER);
