...
error_t mpiExpModFast(Mpi *r, const Mpi *a, const Mpi *e, const Mpi *p)
{
...
#if (BSP_FEATURE_RSIP_SCE9_SUPPORTED != 0)
   //The SCE9 accelerator supports operand lengths up to 4096 bits
...
#else
   //The SCE7 accelerator supports operand lengths up to 2048 bits
   if(aLen <= 256 && eLen <= 4 && pLen <= 256)
   {
      ...
      //Format message representative
      mpiWriteRaw(a, (uint8_t *) rsaArgs.m, 256);

      //Format public key
      mpiWriteRaw(p, (uint8_t *) rsaArgs.key, 256);
      mpiWriteRaw(e, (uint8_t *) rsaArgs.key + 256, 4);
      ...
      {
         //Copy the ciphertext representative
         error = mpiReadRaw(r, (uint8_t *) rsaArgs.c, 256);
      }
      else
      {
         //Report an error
...
#if (BSP_FEATURE_RSIP_SCE9_SUPPORTED != 0)
   //The SCE9 accelerator supports operand lengths up to 2048 bits
