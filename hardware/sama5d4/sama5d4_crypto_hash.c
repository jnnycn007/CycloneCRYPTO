/**
 * @file sama5d4_crypto_hash.c
 * @brief SAMA5D4 hash hardware accelerator
 *
 * @section License
 *
...
#include "hardware/sama5d4/sama5d4_crypto_hash.h"
#include "hash/hash_algorithms.h"
#include "debug.h"

//Check crypto library configuration
#if (SAMA5D4_CRYPTO_HASH_SUPPORT == ENABLED)
   ...
   uint32_t *h)
{
   ...
   osAcquireMutex(&sama5d4CryptoMutex);
   ...
   if(algo == SHA_MR_ALGO_SHA1)
   {
      SHA->SHA_IDATAR[0] = HTOBE32(h[0]);
      SHA->SHA_IDATAR[1] = HTOBE32(h[1]);
      SHA->SHA_IDATAR[2] = HTOBE32(h[2]);
      SHA->SHA_IDATAR[3] = HTOBE32(h[3]);
      SHA->SHA_IDATAR[4] = HTOBE32(h[4]);
   }
   else if(algo == SHA_MR_ALGO_SHA256)
   {
      SHA->SHA_IDATAR[0] = HTOBE32(h[0]);
      SHA->SHA_IDATAR[1] = HTOBE32(h[1]);
      SHA->SHA_IDATAR[2] = HTOBE32(h[2]);
      SHA->SHA_IDATAR[3] = HTOBE32(h[3]);
      SHA->SHA_IDATAR[4] = HTOBE32(h[4]);
      SHA->SHA_IDATAR[5] = HTOBE32(h[5]);
      SHA->SHA_IDATAR[6] = HTOBE32(h[6]);
      SHA->SHA_IDATAR[7] = HTOBE32(h[7]);
   }
   else
   {
      SHA->SHA_IDATAR[0] = HTOBE32(h[1]);
      SHA->SHA_IDATAR[1] = HTOBE32(h[0]);
      SHA->SHA_IDATAR[2] = HTOBE32(h[3]);
      SHA->SHA_IDATAR[3] = HTOBE32(h[2]);
      SHA->SHA_IDATAR[4] = HTOBE32(h[5]);
      SHA->SHA_IDATAR[5] = HTOBE32(h[4]);
      SHA->SHA_IDATAR[6] = HTOBE32(h[7]);
      SHA->SHA_IDATAR[7] = HTOBE32(h[6]);
      SHA->SHA_IDATAR[8] = HTOBE32(h[9]);
      SHA->SHA_IDATAR[9] = HTOBE32(h[8]);
      SHA->SHA_IDATAR[10] = HTOBE32(h[11]);
      SHA->SHA_IDATAR[11] = HTOBE32(h[10]);
      SHA->SHA_IDATAR[12] = HTOBE32(h[13]);
      SHA->SHA_IDATAR[13] = HTOBE32(h[12]);
      SHA->SHA_IDATAR[14] = HTOBE32(h[15]);
      SHA->SHA_IDATAR[15] = HTOBE32(h[14]);
   }
   ...
   osReleaseMutex(&sama5d4CryptoMutex);
}
