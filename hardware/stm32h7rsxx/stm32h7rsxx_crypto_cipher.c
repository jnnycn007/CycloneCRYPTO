/**
 * @file stm32h7rsxx_crypto_cipher.c
...
#if (STM32H7RSXX_CRYPTO_CIPHER_SUPPORT == ENABLED && AES_SUPPORT == ENABLED)
   ...
   uint8_t *output, size_t length, uint32_t mode)
{
   ...
   osAcquireMutex(&stm32h7rsxxCryptoMutex);
   ...
   osReleaseMutex(&stm32h7rsxxCryptoMutex);
}
