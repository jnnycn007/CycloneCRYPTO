/**
 * @file stm32h7xx_crypto_cipher.c
 * @brief STM32H7 cipher hardware accelerator
 *
 * @section License
 *
...
#if (STM32H7XX_CRYPTO_CIPHER_SUPPORT == ENABLED)
...
#endif
   {
      //For revision B and above, the data is written without swapping
      m = aLen * 8;
   ...
   uint8_t *t, uint32_t mode)
{
   ...
   if(HAL_GetREVID() < REV_ID_B)
   {
      ...
      CRYP->DIN = LOAD32LE(b0);
      CRYP->DIN = LOAD32LE(b0 + 4);
      CRYP->DIN = LOAD32LE(b0 + 8);
      CRYP->DIN = LOAD32LE(b0 + 12);
   }
   else
#endif
   {
      //For revision B and above, B0 is written without swapping
   ...
   if(HAL_GetREVID() < REV_ID_B)
   {
      ...
      CRYP->DIN = LOAD32LE(buffer);
      CRYP->DIN = LOAD32LE(buffer + 4);
      CRYP->DIN = LOAD32LE(buffer + 8);
      CRYP->DIN = LOAD32LE(buffer + 12);
   }
   else
#endif
   {
      //For revision B and above, CTR(0) is written without swapping
